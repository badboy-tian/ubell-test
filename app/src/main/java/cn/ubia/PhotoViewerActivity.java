...
public class PhotoViewerActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.mUriString  =  this.getIntent().getExtras().getString("filename"); //(String) list2.get(position);//
		if (mUriString == null) {
...
//				.build();
		...
		mProgressBar = new MyProgressBar(this);
	 	mPhotoView.setImageBitmap(BitmapFactory.decodeFile(this.mUriString));  
	 	 onLoadingComplete();
	}
		...
		mPhotoView.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View arg0, MotionEvent event) {
				...
				case MotionEvent.ACTION_DOWN:
					...
					Log.v("mode", "ACTION_DOWN mode =" + mode
							+ "defaultMatrix =currentMatrix  ="
							+ (defaultMatrix == currentMatrix));
					if (firstClick != 0
				...
				case MotionEvent.ACTION_MOVE:
					Log.v("mode", "ACTION_MOVE mode =" + mode
							+ "defaultMatrix =currentMatrix  ="
							+ (defaultMatrix == currentMatrix));
					// 拖拉图片
					if (hasscale) {
				...
				case MotionEvent.ACTION_UP:
					Log.v("mode", "ACTION_UP mode =" + mode
							+ "defaultMatrix =currentMatrix  ="
							+ (defaultMatrix == currentMatrix));
					...
					if (!hasscale) {
						 float dx = event.getX() - startPoint.x;
						   if(dx<-100)
						 {
						 //Toast.makeText( PhotoViewActivity.this, "下一张",
//						 Toast.LENGTH_SHORT).show();
						 position++ ;
						 if(position>list2.size()-1)
						 position =0;
						 list2.get( position);
//						 ImageLoader.getInstance().displayImage((String)list2.get(
//						 position ),mPhotoView);
							mPhotoView.setImageBitmap(BitmapFactory.decodeFile((String)list2.get(
									 position )));  
							matrix.set(defaultMatrix);
							hasscale = false;
						 }
						 ...
						 position =list2.size()-1;
							...
							matrix.set(defaultMatrix);
							hasscale = false;
						 //Toast.makeText( PhotoViewActivity.this, "上一张",
//						 Toast.LENGTH_SHORT).show();
						 }
				...
				case MotionEvent.ACTION_POINTER_DOWN:
					Log.v("mode", " ACTION_POINTER_DOWN mode =" + mode
							+ "defaultMatrix =currentMatrix  ="
							+ (defaultMatrix == currentMatrix));
					mode = MODE_ZOOM;
					/** 计算两个手指间的距离 */
	...
	private void showToast(CharSequence text) {
		...
		mCurrentToast = Toast.makeText(PhotoViewerActivity.this, text,
				Toast.LENGTH_SHORT);
		mCurrentToast.show();
	}
